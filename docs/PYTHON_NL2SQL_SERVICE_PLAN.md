...
- **Move the authoritative guard into Python** as `ceiba_nl2sql/guard/sql_guard.py`, re-implemented on **`sqlglot`** (real dialect-aware parse) rather than the hand tokenizer, and is *shared* with prep's exemplar validation and the eval scorer. The Python guard runs inside `/nl2sql/generate`'s self-repair loop (the `[E] guardSql` step) exactly as `generate.ts` does today. **Relationship to the TS guard (corrected):** the two guards are NOT automatically "strictly stronger" — they are two independent implementations (sqlglot AST vs. a leading-keyword tokenizer) **kept in sync by hand-maintained verb/denylist lists plus a lexical fallback in the Python guard**, and the *intersection* of what they reject is intentional. The Python guard adds an AST-level write/DDL walk (catching writes buried in a CTE body) AND a fail-closed **lexical fallback** (a write-verb scan and a large-table word-boundary scan over comment-stripped SQL) precisely so it is **at least as strict as** the TS guard on every case in the shared adversarial corpus. That relationship is enforced by a **parity test** (`tests/unit/sqlGuardParity.test.ts` + `ceiba_nl2sql/tests/test_sqlguard_parity.py`) that runs one shared corpus (`tests/fixtures/sqlGuardParityCorpus.json`) through BOTH guards and asserts the Python guard rejects everything the TS guard rejects. Both guards also reject filesystem/network table functions (`read_csv`/`read_parquet`/`read_text`/`glob`/`COPY … TO`/`SELECT … INTO`, etc.) as defense in depth behind the engine's runtime `enable_external_access=false` lockdown.
...
Feature flag: `NL2SQL_RUNTIME` env, values `ts` (default, today) | `python`. This is the UMBRELLA flag both endpoints inherit. Per-endpoint sub-flags (`NL2SQL_GENERATE_RUNTIME`, `NL2SQL_QUERY_RUNTIME`) override the umbrella and allow cutting over generate before execute during transition. Resolution + a one-time boot WARNING on divergence live in `lib/nl2sqlRuntime.ts` (`warnIfRuntimesDiverge`, §7.3). **Rename note:** the query-side flag is `NL2SQL_QUERY_RUNTIME`; an earlier draft of this plan called it `NL2SQL_EXECUTE_RUNTIME`, but the code has always used `NL2SQL_QUERY_RUNTIME` — the plan is corrected to match.
...
- **New env vars:** `NL2SQL_SERVICE_URL` + `NL2SQL_SERVICE_TOKEN` (both processes), `NL2SQL_SERVICE_PORT` (service), `NL2SQL_RUNTIME` / `NL2SQL_GENERATE_RUNTIME` / `NL2SQL_QUERY_RUNTIME` (TS route flags).
...
| **4. Cut over execute** | `query` route calls `/nl2sql/execute` when `NL2SQL_QUERY_RUNTIME=python`; DB creds move to service; TS guard/audit order preserved | Full NL→SQL path via Python for flagged traffic; dialect-alignment contract test green (one engine); audit chain still records QUERY_RUN in TS | `NL2SQL_QUERY_RUNTIME=ts` (env flip); DB creds still present in Next while `=ts` |
